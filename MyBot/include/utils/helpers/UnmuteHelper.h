...
CheckResult get_target_member(dpp::interaction command, UnmuteData& data);								// Fetch the guild_member object of the target user and set it in the MuteData struct. Return : See CheckResult struct for more details
CheckResult check_can_unmute(const UnmuteData& data, dpp::snowflake& bot_id);								// Check if the mute action can be performed based on permissions and other factors. Return : See CheckResult struct for more details
dpp::task<void> co_send_unmute_interaction_reply(const dpp::slashcommand_t& event, const UnmuteData& data);	// Edit the original command message with the mute success reply. Return : nothing
void send_unmute_log_message(dpp::cluster& bot, const UnmuteData& data);									// Send a log message in the defined log channel with the details of the mute. Return : nothing
void fetch_and_set_user_permissions(const dpp::interaction interaction, UnmuteData& data);				// Fetch the permissions of the issuer, target and bot and set them in the MuteData struct. Return : nothing
